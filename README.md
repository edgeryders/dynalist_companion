...
6. Save the secret code generated by `setup.py` to share it later with those you want to allow to registrater.
    
    You can change the secret code at any time by changing the value of `SECRET_CODE` in `config.py`.
...
7. In `config.py`, enter the file ID of the Dynalist file that Dynalist Companion should observe.
